...
def get_dynamic_personality(v_ego, personality=custom.LongitudinalPersonalitySP.standard):
  if personality==custom.LongitudinalPersonalitySP.relaxed:
    ...
    x_vel =  [0,     5,     5.01,  15,    20,    20.01,  25, 25.01, 36,   36.01]
    y_dist = [0.81,  0.80,  1.16,  1.15, 1.25,  1.25, 1.25, 1.25, 1.25,  1.25]
  else:
    raise NotImplementedError("Dynamic personality not supported")
...
class LongitudinalMpc:
  ...
  def update(self, radarstate, v_cruise, prev_accel_constraint, x, v, a, j, personality=custom.LongitudinalPersonalitySP.standard, dynamic_personality=False, fast_take_off = False):
    v_ego = self.x0[1]
